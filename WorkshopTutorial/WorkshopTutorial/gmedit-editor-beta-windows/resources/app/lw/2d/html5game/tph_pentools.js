...
function mark_init() {
	...
	//
	var instGet = Object.create(null);
	var instSet = Object.create(null);
	...
	js.replace(/mark_qget\(\s*"(\w+)",\s*(.+?)\s*\);/g, function(r, name, expr) {
		var k = "get$" + name; found.push(k);
		getCode[name] = expr;
		var fn = new Function(inst, "return " + expr + ";");
		instGet[name] = fn;
		window[k] = fn;
		return r;
	});
	...
	js.replace(/mark_qset\(\s*"(\w+)"\s*\);\s*(.+?;)/g, function(r, name, expr) {
		...
		k = "aop$" + name; found.push(k);
		var fn = new Function(inst, "__op__", val,
			val + " = gml$op(" + getCode[name] + ", __op__, " + val + "); "
			+ expr + "; return " + val + ";"
		);
		instSet[name] = fn;
	...
	window.get$current_time = function() {
		return 0|(performance.now() - window.get$current_time_rel);
	}
	window.gml$get_timer = function() {
		return (performance.now() - window.get$current_time_rel) * 1000;
	}
	//}
	//{ Sprite
	wrap_next("sprite");
	...
	//}
	//{ Variable
	var instRaw = window.gml_Script_gmcallback_resolve;
	window.gml$variable_instance_exists = function(inst, field) {
	    if (inst == null) return false;
	    if (typeof inst != "object") {
	        inst = instRaw(null, null, inst);
	        if (inst == null) return false;
	        if (instGet[field]) return true;
	    }
	    return ("js$" + field) in inst;
	}
	window.gml$variable_instance_get = function(inst, field) {
	    if (inst == null) throw "Instance doesn't exist";
	    if (typeof inst != "object") {
	        inst = instRaw(null, null, inst);
	        if (inst == null) throw "Instance doesn't exist";
	    }
	    var fn = instGet[field];
	    return fn ? fn(inst) : inst["js$" + field];
	}
	window.gml$variable_instance_set = function(inst, field, value) {
	    if (inst == null) throw "Instance doesn't exist";
	    if (typeof inst != "object") {
	        inst = instRaw(null, null, inst);
	        if (inst == null) throw "Instance doesn't exist";
	    }
	    var fn = instSet[field];
	    if (fn) {
	        fn(inst, value);
	    } else inst["js$" + field] = value;
	}
	//}
	//{ Drawing
	...
	window.gml$game_prepare = function() {
		window.get$current_time_rel = performance.now();
...
traceCss.innerHTML = [
	...
	"#tracelist li .string { color: rgb(255, 128, 128); white-space: pre }",
].join("\n");
